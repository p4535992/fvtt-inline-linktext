...
export class HTMLEnricherTriggers {
  ...
  static enrich(text, id, label, entity, iconClass) {
    ...
    //   throw new Error(game.i18n.localize('InvalidFormat for inline linktext'))
    // }
    let noId = false;
    ...
    if (lBracket === -1 || rBracket === -1) {
      // new Error(game.i18n.localize('InvalidFormat for inline linktext'));
      const result = `<a class="entity-link broken" draggable="true" data-entity="inline-linktext" data-id="null"><i class="fas fa-unlink"></i> ${text}</a>`;
      return result;
    }
